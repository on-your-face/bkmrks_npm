...
P.S. Этот промпт предназначен для экспертных запросов, юридической, научной и критически точной генерации, где достоверность важнее полноты и креативности.`}[s];if(n){const e=document.createElement("textarea");e.style.position="fixed",e.style.opacity="0",e.value=n,document.body.appendChild(e),e.focus(),e.select(),document.execCommand("copy"),document.body.removeChild(e)}}})})}function f(){console.log("initCopySmall: скрипт инициализирован"),document.querySelectorAll("a.copy").forEach(t=>{console.log("initCopySmall: найден элемент",t),t.addEventListener("click",r=>{r.preventDefault(),r.stopPropagation(),history.pushState("",document.title,window.location.pathname+window.location.search),console.log("initCopySmall: клик по элементу",t);const s=t.getAttribute("href");if(!s){console.warn("initCopySmall: href пустой");return}navigator.clipboard.writeText(s).then(()=>{console.log(`initCopySmall: скопировано в буфер — ${s}`)}).catch(a=>{console.error("initCopySmall: ошибка копирования",a)})})})}async function h(){const t=await fetch("./html/main.html");if(!t.ok){console.error("Ошибка загрузки main.html:",t.status);return}const r=await t.text();document.getElementById("main-container").innerHTML=r}async function y(){const t=await fetch("./html/modals.html");if(!t.ok){console.error("Ошибка загрузки modals.html:",t.status);return}const r=await t.text();document.getElementById("modals-container").innerHTML=r,console.log("modals.html загружен и вставлен")}async function _(){await h(),await y(),u(),p(),g(),f()}_();
