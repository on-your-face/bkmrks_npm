...
P.S. Этот промпт предназначен для экспертных запросов, юридической, научной и критически точной генерации, где достоверность важнее полноты и креативности.`}[s];if(t){const e=document.createElement("textarea");e.style.position="fixed",e.style.opacity="0",e.value=t,document.body.appendChild(e),e.focus(),e.select(),document.execCommand("copy"),document.body.removeChild(e)}}})})}function f(){console.log("initCopySmall: скрипт инициализирован"),document.querySelectorAll("a.copy").forEach(o=>{console.log("initCopySmall: найден элемент",o),o.addEventListener("click",r=>{r.preventDefault(),r.stopPropagation(),history.pushState("",document.title,window.location.pathname+window.location.search),console.log("initCopySmall: клик по элементу",o);const s=o.getAttribute("href");if(!s){console.warn("initCopySmall: href пустой");return}navigator.clipboard.writeText(s).then(()=>{console.log(`initCopySmall: скопировано в буфер — ${s}`)}).catch(a=>{console.error("initCopySmall: ошибка копирования",a)})})})}async function h(){const o=["1rem OpenSans","1rem Lora-Regular"];await Promise.all(o.map(r=>document.fonts.load(r)))}m(),u(),g(),f(),h();
