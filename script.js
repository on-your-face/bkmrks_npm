async function loadFont(e, t, n) {
  const o =
      t['woff2' in document.createElement('link').relList ? 'woff2' : 'woff'],
...
async function preloadFonts() {
  await Promise.all([
    loadFont(
      'OpenSans',
      {
        woff: './assets/fonts/OpenSans-Light.woff',
        woff2: './assets/fonts/OpenSans-Light.woff2',
      },
    ...
    loadFont(
      'Lora-Regular',
      {
        woff: './assets/fonts/Lora-Regular.woff',
        woff2: './assets/fonts/Lora-Regular.woff2',
      },
      { style: 'normal', weight: '400' }
    ),
  ]);
...
function initModal() {
  ...
  const t = document.querySelectorAll('a.anchor[href^="#"]'),
    ...
    o = () => {
      (n.forEach((e) => e.classList.remove('open__hashtags')),
        t.forEach((e) => (e.style.opacity = '')));
    };
  t.forEach((e) => {
    e.addEventListener('click', (t) => {
      ...
      (o(), r || (a.classList.add('open__hashtags'), (e.style.opacity = '1')));
    });
  });
  let a = 0,
    r = 0,
  ...
  (document.addEventListener('touchstart', (e) => {
    ...
    t && ((a = e.touches[0].clientX), (s = !0), (t.dataset.isSwiping = 'true'));
  }),
    ...
    e.addEventListener('click', function (e) {
      ...
      if (t) {
        const e = {
          ...
          'cmd-OSP-server-START':
            'start "" "c:\\OSPanel\\Open Server Panel.exe" /start',
          'cmd-OSP-server-RESTART':
            'start "" "c:\\OSPanel\\Open Server Panel.exe" /restart',
          'cmd-OSP-server-STOP':
            'start "" "c:\\OSPanel\\Open Server Panel.exe" /stop',
          ...
          'close process (OSP promt)':
            'taskkill /IM "Open Server Panel.exe" /F',
          termux_iam_true:
            'git config --global --add safe.directory /storage/emulated/0/bookmarks',
          ...
          'directory-buttery-taskbar':
            'C:\\Users\\user\\AppData\\Roaming\\Microsoft\\Windows\\Start Menu\\Programs\\Cryptic Butter',
          gpt_galuni:
